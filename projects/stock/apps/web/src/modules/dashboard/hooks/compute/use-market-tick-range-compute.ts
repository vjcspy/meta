...
import { useTickDailyQueries } from "@/modules/dashboard/hooks/queries/use-tick-daily-queries";
import { useVNIndexPriceRange } from "@/modules/dashboard/hooks/queries/use-vnindex-price-range";
...
export function useMarketTickRangeCompute() {
  ...
  const tickQueries = useTickDailyQueries(symbols, fromDate, toDate, enabled, { staleTime: 5 * 60 * 1000 });
  ...
  useVNIndexPriceRange(fromDate, toDate, enabled);
