...
import type { WorkerInput, WorkerOutput } from "@/modules/dashboard/utils/types";
...
ctx.onmessage = (event: MessageEvent<WorkerInput>) => {
  ...
  const results = symbolTicks.map(({ symbol, ticks }) => calTickRangeData({ symbol, ticks, tradeValue }));
