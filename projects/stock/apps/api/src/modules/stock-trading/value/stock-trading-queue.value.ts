export const STOCK_TRADING_EXCHANGE_KEY = 'stock.trading.exchange';
export const STOCK_TRADING_STRATEGY_ROUTING_KEY = 'stock.trading.strategy';

export const STOCK_TRADING_ANALYSIS_EXCHANGE_KEY =
  'stock.trading.analysis.exchange';
export const STOCK_TRADING_ANALYSIS_JOB_KEY =
  'stock.trading.analysis.job.queue.key';

export const STOCK_TRADING_ANALYSIS_DEFAULT_CAT_NODEJS_JOB_KEY =
  'stock.trading.analysis.defaultcat.nodejs.job.queue.key';

export const STOCK_TRADING_ANALYSIS_DEFAULT_CAT_NODEJS_QUEUE =
  'stock.trading.analysis.defaultcat.nodejs.queue';
export const STOCK_TRADING_WORKER_ANALYSIS_KEY = 'stock_trading_analysis';
export const STOCK_TRADING_WORKER_ANALYSIS_HULLMA_KEY =
  'stock_trading_analysis_hullma';

export const MARKET_ACTION_INFO_EXCHANGE =
  'stock.trading.market.tick.action.analysis.exchange';

export const MARKET_ACTION_INFO_JOB_KEY =
  'stock.trading.market.tick.action.analysis.job.key';

export const MARKET_ACTION_INFO_QUEUE =
  'stock.trading.market.tick.action.analysis.queue';

export const MARKET_ACTION_HISTORY_ANALYZE_JOB_KEY =
  'stock.trading.market.tick.history.analysis.job.key';

export const MARKET_ACTION_HISTORY_ANALYZE_QUEUE =
  'stock.trading.market.tick.history.analysis.queue';

export const MARKET_ACTION_DAY_ANALYZE_JOB_KEY =
  'stock.trading.market.tick.day.analysis.job.key';

export const MARKET_ACTION_DAY_ANALYZE_QUEUE =
  'stock.trading.market.tick.day.analysis.queue';
