...
export const loadIntraDaySpeed$ = createEffect((action$, state$) =>
  action$.pipe(
    ...
    switchMap((value: any) => {
      ...
      const url = `${process.env.NEXT_PUBLIC_ENDPOINT_LIVE_URL}/market-tick-action/intra-day-speed?symbol=${analysisState.intraDaySpeedSymbol}&date=${analysisState.toDate}`;
